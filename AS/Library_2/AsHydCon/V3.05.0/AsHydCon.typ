TYPE
	HCRMovePara_typ : STRUCT
		t_Jolt	: REAL ;
		vC_pos	: REAL ;
		vC_neg	: REAL ;
		aAcc_pos	: REAL ;
		aDec_pos	: REAL ;
		aAcc_neg	: REAL ;
		aDec_neg	: REAL ;
	END_STRUCT;
	HCRSFG_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		SFG_Para	: HCRMovePara_typ ;
		Profile_Case	: USINT ;
		Fp_delta	: REAL ;
		Fp_brake	: REAL ;
		Fp_set_0	: REAL ;
		dFp_set_0	: REAL ;
		dFp_home	: REAL ;
		dFp_0_intrude	: USINT ;
		dFp_const	: REAL ;
		dFp_const_avg	: REAL ;
		dFp_thresh	: ARRAY[0..2] OF REAL ;
		t_Jolt	: REAL ;
		t_delta	: REAL ;
		t_delta_Jolt	: REAL ;
		t_delta_3_Jolt	: REAL ;
		Calc_t_delta	: USINT ;
		tx	: REAL ;
		tx_limit	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRF_Cont_InVar_typ : STRUCT
		I_Switch	: USINT ;
		Q_man	: REAL ;
		Q_add	: REAL ;
		v_act	: REAL ;
		Qy_Oset_ext	: REAL ;
		Oset_Mode	: USINT ;
	END_STRUCT;
	HCRP2P_Cont_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		MoveDir	: SINT ;
		s_set_old	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRP2P_Cont_AddPar_typ : STRUCT
		Y_P2P_min	: REAL ;
		Y_P2P_max	: REAL ;
		dY_max	: REAL ;
		Tn_pos	: REAL ;
		Tn_neg	: REAL ;
		I_Switch	: USINT ;
		Tv	: REAL ;
		Tf	: REAL ;
	END_STRUCT;
	HCRF_Cont_p_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		Oset_Mode	: USINT ;
		Qy_Oset_F_pos	: REAL ;
		Qy_Oset_F_neg	: REAL ;
		PID_F_bias	: USINT ;
		Qy_F_bias	: REAL ;
		yValve_bias	: REAL ;
		K_yValve	: REAL ;
		dF_Cyl_bias	: REAL ;
		dF_Cyl_v	: REAL ;
		dF_Cyl_sum	: REAL ;
		dF_Cyl_abs	: REAL ;
		dF_Cyl_rel	: REAL ;
		Y_bias	: REAL ;
		MoveDir	: SINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRFyServo_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		E	: REAL ;
		Alpha	: REAL ;
		pSupply_max	: REAL ;
		pSupply	: REAL ;
		pTank	: REAL ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
		V0_pos	: REAL ;
		V0_neg	: REAL ;
		Neg_y	: BOOL ;
		yValve_0	: REAL ;
		dF_Cyl_sup	: REAL ;
		dF_Cyl_sum	: REAL ;
		x_Cyl	: REAL ;
		h_Cyl	: REAL ;
		h_Cyl_dbl_1	: UDINT ;
		h_Cyl_dbl_2	: UDINT ;
		K_pos_dbl_1	: UDINT ;
		K_pos_dbl_2	: UDINT ;
		K_neg_dbl_1	: UDINT ;
		K_neg_dbl_2	: UDINT ;
		K_yValve	: REAL ;
		K_yValve_dbl_1	: UDINT ;
		K_yValve_dbl_2	: UDINT ;
		V0_A_Cyl_pos_dbl_1	: UDINT ;
		V0_A_Cyl_pos_dbl_2	: UDINT ;
		V0_A_Cyl_neg_dbl_1	: UDINT ;
		V0_A_Cyl_neg_dbl_2	: UDINT ;
		A_Cyl_pos_1000_dbl_1	: UDINT ;
		A_Cyl_pos_1000_dbl_2	: UDINT ;
		A_Cyl_neg_1000_dbl_1	: UDINT ;
		A_Cyl_neg_1000_dbl_2	: UDINT ;
		E_10_dbl_1	: UDINT ;
		E_10_dbl_2	: UDINT ;
		pDelta_min_dbl_1	: UDINT ;
		pDelta_min_dbl_2	: UDINT ;
		dF_Cyl_sup_100_dbl_1	: UDINT ;
		dF_Cyl_sup_100_dbl_2	: UDINT ;
		K_v_Cyl	: REAL ;
		Update_old	: USINT ;
		Update_2	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRNotch_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		fs_dbl_1	: UDINT ;
		fs_dbl_2	: UDINT ;
		V_old	: REAL ;
		f0_old	: REAL ;
		B_old	: REAL ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		fg_upper	: REAL ;
		fg_lower	: REAL ;
		B_act	: REAL ;
		D	: REAL ;
		Tf	: REAL ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		D_Ts_Tf_dbl_1	: UDINT ;
		D_Ts_Tf_dbl_2	: UDINT ;
		Sqrt_D_1_dbl_1	: UDINT ;
		Sqrt_D_1_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		a1_dbl_1	: UDINT ;
		a1_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		b1_dbl_1	: UDINT ;
		b1_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRIT1_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		V_old	: REAL ;
		Tn_old	: REAL ;
		T1_old	: REAL ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		Exp_Ts_T1_dbl_1	: UDINT ;
		Exp_Ts_T1_dbl_2	: UDINT ;
		V_Tn_dbl_1	: UDINT ;
		V_Tn_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		a1_dbl_1	: UDINT ;
		a1_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		b1_dbl_1	: UDINT ;
		b1_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRSystemParaAdd_typ : STRUCT
		V0_pos	: REAL ;
		V0_neg	: REAL ;
		h_Cyl	: REAL ;
		m_Cyl	: REAL ;
	END_STRUCT;
	HCRRampGen_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		t_rise	: REAL ;
		t_hold	: REAL ;
		t_fall	: REAL ;
		y_start	: REAL ;
		y_hold	: REAL ;
		y_end	: REAL ;
		y	: REAL ;
		dy	: REAL ;
		Finished	: BOOL ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		Timer_dbl_1	: UDINT ;
		Timer_dbl_2	: UDINT ;
		k_Ramp_dbl_1	: UDINT ;
		k_Ramp_dbl_2	: UDINT ;
		y_dbl_1	: UDINT ;
		y_dbl_2	: UDINT ;
		RampGenPhase	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRAutoTune_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		E	: REAL ;
		V0_pos	: REAL ;
		V0_pos_mod_dbl_1	: UDINT ;
		V0_pos_mod_dbl_2	: UDINT ;
		V0_neg	: REAL ;
		V0_neg_mod_dbl_1	: UDINT ;
		V0_neg_mod_dbl_2	: UDINT ;
		s_Cyl_min_dbl_1	: UDINT ;
		s_Cyl_min_dbl_2	: UDINT ;
		s_Cyl_max_dbl_1	: UDINT ;
		s_Cyl_max_dbl_2	: UDINT ;
		h_Cyl	: REAL ;
		h_Cyl_dbl_1	: UDINT ;
		h_Cyl_dbl_2	: UDINT ;
		E_A_pos_2_dbl_1	: UDINT ;
		E_A_pos_2_dbl_2	: UDINT ;
		E_A_neg_2_dbl_1	: UDINT ;
		E_A_neg_2_dbl_2	: UDINT ;
		s0_Cyl	: REAL ;
		s0_Cyl_dbl_1	: UDINT ;
		s0_Cyl_dbl_2	: UDINT ;
		c0_Cyl	: REAL ;
		c0_Cyl_dbl_1	: UDINT ;
		c0_Cyl_dbl_2	: UDINT ;
		omega0_Cyl	: REAL ;
		omega0_Cyl_dbl_1	: UDINT ;
		omega0_Cyl_dbl_2	: UDINT ;
		vCyl_sup_pos	: REAL ;
		vCyl_sup_neg	: REAL ;
		vCyl_sup	: REAL ;
		Kp_dbl_1	: UDINT ;
		Kp_dbl_2	: UDINT ;
		Kp_physical	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRs_Cont_p_InVar_typ : STRUCT
		I_Switch	: USINT ;
		v_man	: REAL ;
		v_add	: REAL ;
		v_set	: REAL ;
		pCyl_pos	: REAL ;
		pCyl_neg	: REAL ;
		Qy_Oset_ext	: REAL ;
		Oset_Mode	: USINT ;
	END_STRUCT;
	HCRsF_Cont_p_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		CTRLmode	: USINT ;
		F_Cont_d_mode	: USINT ;
		F_Cont_Qy	: REAL ;
		PID_s_bias	: USINT ;
		Qy_s_bias	: REAL ;
		PID_F_bias	: USINT ;
		Qy_F_bias	: REAL ;
		tSwitch	: REAL ;
		uSwitch_old	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRs_Cont_p_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		vCyl_sup_pos	: REAL ;
		vCyl_sup_neg	: REAL ;
		vCyl_sup	: REAL ;
		s_set_old	: REAL ;
		s_act_old	: REAL ;
		v_act	: REAL ;
		PID_s_bias	: USINT ;
		Qy_s_bias	: REAL ;
		yValve_bias	: REAL ;
		K_yValve	: REAL ;
		vCyl_bias	: REAL ;
		vCyl_sum	: REAL ;
		vCyl_abs	: REAL ;
		vCyl_rel	: REAL ;
		Y_bias	: REAL ;
		Oset_Mode	: USINT ;
		Qy_Oset_s_pos	: REAL ;
		Qy_Oset_s_neg	: REAL ;
		Qy_I_Switch	: USINT ;
		Qy_I	: REAL ;
		MoveDir_I	: SINT ;
		vMode	: USINT ;
		v_lim	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCROmega0_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		E	: REAL ;
		Kp_xCorr_Lim	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
		V0_pos	: REAL ;
		V0_neg	: REAL ;
		h_Cyl	: REAL ;
		x_Cyl_min_dbl_1	: UDINT ;
		x_Cyl_min_dbl_2	: UDINT ;
		x_Cyl_max_dbl_1	: UDINT ;
		x_Cyl_max_dbl_2	: UDINT ;
		E_A_pos_2_dbl_1	: UDINT ;
		E_A_pos_2_dbl_2	: UDINT ;
		E_A_neg_2_dbl_1	: UDINT ;
		E_A_neg_2_dbl_2	: UDINT ;
		x0_Cyl_dbl_1	: UDINT ;
		x0_Cyl_dbl_2	: UDINT ;
		c0_Cyl	: REAL ;
		c0_Cyl_dbl_1	: UDINT ;
		c0_Cyl_dbl_2	: UDINT ;
		c_Cyl_Lim_L	: REAL ;
		c_Cyl_Lim_L_dbl_1	: UDINT ;
		c_Cyl_Lim_L_dbl_2	: UDINT ;
		c_Cyl_Lim_R	: REAL ;
		c_Cyl_Lim_R_dbl_1	: UDINT ;
		c_Cyl_Lim_R_dbl_2	: UDINT ;
		c_Cyl	: REAL ;
		c_Cyl_dbl_1	: UDINT ;
		c_Cyl_dbl_2	: UDINT ;
		Kp_xCorr_Lim_L	: REAL ;
		Kp_xCorr_Lim_R	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRLinValve_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Neg_Qy	: BOOL ;
		Neg_Uy	: BOOL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRFIFO_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		t_FIFO_max	: DINT ;
		MemSize	: UDINT ;
		MemSize_REAL	: UDINT ;
		Set_y_2	: BOOL ;
		pFIFO_Buf	: UDINT ;
		pFIFO_OutIn	: UDINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRDT2_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		Tv_old	: REAL ;
		T1_old	: REAL ;
		T2_old	: REAL ;
		n_Avg_old	: USINT ;
		u_Buf	: ARRAY[0..99] OF REAL ;
		u_BufCnt	: SINT ;
		u_Sum_dbl_1	: UDINT ;
		u_Sum_dbl_2	: UDINT ;
		u_Avg_dbl_1	: UDINT ;
		u_Avg_dbl_2	: UDINT ;
		Reset_y_old	: USINT ;
		Reset_y_2	: USINT ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		Tv_T1_dbl_1	: UDINT ;
		Tv_T1_dbl_2	: UDINT ;
		Tv_T2_dbl_1	: UDINT ;
		Tv_T2_dbl_2	: UDINT ;
		Exp_Ts_T1_dbl_1	: UDINT ;
		Exp_Ts_T1_dbl_2	: UDINT ;
		Exp_Ts_T2_dbl_1	: UDINT ;
		Exp_Ts_T2_dbl_2	: UDINT ;
		Exp_T1_T2_dbl_1	: UDINT ;
		Exp_T1_T2_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		a1_dbl_1	: UDINT ;
		a1_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		b1_dbl_1	: UDINT ;
		b1_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRF_Cont_p_InVar_typ : STRUCT
		I_Switch	: USINT ;
		Q_man	: REAL ;
		Q_add	: REAL ;
		dFp_set	: REAL ;
		x_Cyl	: REAL ;
		v_Cyl	: REAL ;
		pCyl_pos	: REAL ;
		pCyl_neg	: REAL ;
		v_act	: REAL ;
		Qy_Oset_ext	: REAL ;
		Oset_Mode	: USINT ;
	END_STRUCT;
	HCRs_Cont_InVar_typ : STRUCT
		I_Switch	: USINT ;
		v_man	: REAL ;
		v_add	: REAL ;
		v_set	: REAL ;
		Fp_Cyl	: REAL ;
		Ff_Cyl_pos	: REAL ;
		Ff_Cyl_neg	: REAL ;
		Qy_Oset_ext	: REAL ;
		Oset_Mode	: USINT ;
	END_STRUCT;
	HCRsF_Cont_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		PID_s_bias	: USINT ;
		Qy_s_bias	: REAL ;
		PID_F_bias	: USINT ;
		Qy_F_bias	: REAL ;
		tSwitch	: REAL ;
		uSwitch_old	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRPIDPara_F_typ : STRUCT
		Fp_min	: REAL ;
		Fp_max	: REAL ;
		Kw	: REAL ;
		Kp	: REAL ;
		Tn	: REAL ;
		K_Fbk	: REAL ;
		d_mode	: USINT ;
		Tv	: REAL ;
		Tf	: REAL ;
		dQ_max	: REAL ;
		Q_min	: REAL ;
		Q_max	: REAL ;
	END_STRUCT;
	HCRPIDPara_s_typ : STRUCT
		s_min	: REAL ;
		s_max	: REAL ;
		Kw	: REAL ;
		Kp	: REAL ;
		Tn_pos	: REAL ;
		Tn_neg	: REAL ;
		K_Fbk	: REAL ;
		d_mode	: USINT ;
		Tv	: REAL ;
		Tf	: REAL ;
		dv_max	: REAL ;
		v_min	: REAL ;
		v_max	: REAL ;
	END_STRUCT;
	HCRF_Cont_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		Oset_Mode	: USINT ;
		Qy_Oset_F_pos	: REAL ;
		Qy_Oset_F_neg	: REAL ;
		PID_F_bias	: USINT ;
		Qy_F_bias	: REAL ;
		yValve_bias	: REAL ;
		K_yValve	: REAL ;
		u_bias	: REAL ;
		u_sum	: REAL ;
		u_abs	: REAL ;
		u_rel	: REAL ;
		Y_bias	: REAL ;
		MoveDir	: SINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRJoltFree_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		uSwitch_old	: BOOL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRDT2s_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		D_old	: REAL ;
		Tv_old	: REAL ;
		Tf_old	: REAL ;
		Reset_y_old	: USINT ;
		Reset_y_2	: USINT ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		Sqrt_D_1_dbl_1	: UDINT ;
		Sqrt_D_1_dbl_2	: UDINT ;
		Tv_Tf_dbl_1	: UDINT ;
		Tv_Tf_dbl_2	: UDINT ;
		Ts_Tf_dbl_1	: UDINT ;
		Ts_Tf_dbl_2	: UDINT ;
		Exp_D_Ts_Tf_dbl_1	: UDINT ;
		Exp_D_Ts_Tf_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		a1_dbl_1	: UDINT ;
		a1_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		b1_dbl_1	: UDINT ;
		b1_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRPT1cP_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		T1_intern	: REAL ;
		nCnt	: UDINT ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		x_dbl_1	: UDINT ;
		x_dbl_2	: UDINT ;
		Exp_5_dbl_1	: UDINT ;
		Exp_5_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRPT2s_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		V_old	: REAL ;
		D_old	: REAL ;
		Tf_old	: REAL ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		Sqrt_D_1_dbl_1	: UDINT ;
		Sqrt_D_1_dbl_2	: UDINT ;
		Ts_Tf_dbl_1	: UDINT ;
		Ts_Tf_dbl_2	: UDINT ;
		Exp_D_Ts_Tf_dbl_1	: UDINT ;
		Exp_D_Ts_Tf_dbl_2	: UDINT ;
		a0_dbl_1	: UDINT ;
		a0_dbl_2	: UDINT ;
		a1_dbl_1	: UDINT ;
		a1_dbl_2	: UDINT ;
		b0_dbl_1	: UDINT ;
		b0_dbl_2	: UDINT ;
		b1_dbl_1	: UDINT ;
		b1_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRPT2_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		V_old	: REAL ;
		T1_old	: REAL ;
		T2_old	: REAL ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		x1_dbl_1	: UDINT ;
		x1_dbl_2	: UDINT ;
		x2_dbl_1	: UDINT ;
		x2_dbl_2	: UDINT ;
		Exp_Ts_T1_dbl_1	: UDINT ;
		Exp_Ts_T1_dbl_2	: UDINT ;
		Exp_Ts_T2_dbl_1	: UDINT ;
		Exp_Ts_T2_dbl_2	: UDINT ;
		Exp_Ts_T1_T2_dbl_1	: UDINT ;
		Exp_Ts_T1_T2_dbl_2	: UDINT ;
		Exp_T1_T2_dbl_1	: UDINT ;
		Exp_T1_T2_dbl_2	: UDINT ;
		V1_Exp_dbl_1	: UDINT ;
		V1_Exp_dbl_2	: UDINT ;
		V2_Exp_dbl_1	: UDINT ;
		V2_Exp_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRPT1_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		V_old	: REAL ;
		T1_old	: REAL ;
		Set_y_old	: USINT ;
		Set_y_2	: USINT ;
		x_dbl_1	: UDINT ;
		x_dbl_2	: UDINT ;
		Exp_Ts_T1_dbl_1	: UDINT ;
		Exp_Ts_T1_dbl_2	: UDINT ;
		V_Exp_dbl_1	: UDINT ;
		V_Exp_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRDT1_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		Tv_old	: REAL ;
		Tf_old	: REAL ;
		n_Avg_old	: USINT ;
		u_Buf	: ARRAY[0..99] OF REAL ;
		u_BufCnt	: SINT ;
		u_Sum_dbl_1	: UDINT ;
		u_Sum_dbl_2	: UDINT ;
		u_Avg_dbl_1	: UDINT ;
		u_Avg_dbl_2	: UDINT ;
		Reset_y_old	: USINT ;
		Reset_y_2	: USINT ;
		x_dbl_1	: UDINT ;
		x_dbl_2	: UDINT ;
		Tv_Tf_dbl_1	: UDINT ;
		Tv_Tf_dbl_2	: UDINT ;
		Exp_Ts_Tf_dbl_1	: UDINT ;
		Exp_Ts_Tf_dbl_2	: UDINT ;
		Tv_Tf_Exp_dbl_1	: UDINT ;
		Tv_Tf_Exp_dbl_2	: UDINT ;
		V_Exp_dbl_1	: UDINT ;
		V_Exp_dbl_2	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRFyServo_Fp_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Alpha	: REAL ;
		pSupply_max	: REAL ;
		pSupply	: REAL ;
		pTank	: REAL ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
		Neg_y	: BOOL ;
		yValve_0	: REAL ;
		Supply_Tank_pos_dbl_1	: UDINT ;
		Supply_Tank_pos_dbl_2	: UDINT ;
		Supply_Tank_neg_dbl_1	: UDINT ;
		Supply_Tank_neg_dbl_2	: UDINT ;
		K_numer_dbl_1	: UDINT ;
		K_numer_dbl_2	: UDINT ;
		K_yValve	: REAL ;
		K_yValve_dbl_1	: UDINT ;
		K_yValve_dbl_2	: UDINT ;
		F_Delta_min_dbl_1	: UDINT ;
		F_Delta_min_dbl_2	: UDINT ;
		Update_old	: USINT ;
		Update_2	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRSPF_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Profile_Nb	: USINT ;
		Profile_ID	: USINT ;
		Profile_Case	: USINT ;
		MovementPhase	: USINT ;
		MovementStep	: USINT ;
		s_end	: REAL ;
		s_set_0_dbl_1	: UDINT ;
		s_set_0_dbl_2	: UDINT ;
		v_set_0_dbl_1	: UDINT ;
		v_set_0_dbl_2	: UDINT ;
		a_set_0_dbl_1	: UDINT ;
		a_set_0_dbl_2	: UDINT ;
		v0	: REAL ;
		v0_dbl_1	: UDINT ;
		v0_dbl_2	: UDINT ;
		v0_intrude	: USINT ;
		v0_state	: USINT ;
		s_set	: REAL ;
		s_set_Jolt	: REAL ;
		s_set_dbl_1	: UDINT ;
		s_set_dbl_2	: UDINT ;
		v_set	: REAL ;
		v_set_Jolt	: REAL ;
		v_set_dbl_1	: UDINT ;
		v_set_dbl_2	: UDINT ;
		a_set	: REAL ;
		a_set_Jolt	: REAL ;
		a_set_dbl_1	: UDINT ;
		a_set_dbl_2	: UDINT ;
		vC_pos_dbl_1	: UDINT ;
		vC_pos_dbl_2	: UDINT ;
		vC_neg_dbl_1	: UDINT ;
		vC_neg_dbl_2	: UDINT ;
		aAcc_pos_dbl_1	: UDINT ;
		aAcc_pos_dbl_2	: UDINT ;
		aDec_pos_dbl_1	: UDINT ;
		aDec_pos_dbl_2	: UDINT ;
		aAcc_neg_dbl_1	: UDINT ;
		aAcc_neg_dbl_2	: UDINT ;
		aDec_neg_dbl_1	: UDINT ;
		aDec_neg_dbl_2	: UDINT ;
		aDec	: REAL ;
		aDec_intrude	: BOOL ;
		s_period	: REAL ;
		s_period_Sum_dbl_1	: UDINT ;
		s_period_Sum_dbl_2	: UDINT ;
		Nb_period	: DINT ;
		t_Jolt_max	: REAL ;
		t_Jolt_limit	: REAL ;
		t_Jolt	: REAL ;
		t_Jolt_dbl_1	: UDINT ;
		t_Jolt_dbl_2	: UDINT ;
		sDelta	: REAL ;
		sDelta_dbl_1	: UDINT ;
		sDelta_dbl_2	: UDINT ;
		sBrake	: REAL ;
		sBrake_dbl_1	: UDINT ;
		sBrake_dbl_2	: UDINT ;
		vTop	: REAL ;
		vTop_dbl_1	: UDINT ;
		vTop_dbl_2	: UDINT ;
		AccVec_dbl	: ARRAY[0..39] OF UDINT ;
		ConstVec_dbl	: ARRAY[0..39] OF UDINT ;
		DecVec_dbl	: ARRAY[0..39] OF UDINT ;
		AccVec	: ARRAY[0..19] OF REAL ;
		ConstVec	: ARRAY[0..19] OF REAL ;
		DecVec	: ARRAY[0..19] OF REAL ;
		pBuf_s_set	: UDINT ;
		pBuf_v_set	: UDINT ;
		pBuf_a_set	: UDINT ;
		BufCntMax	: DINT ;
		BufCnt	: DINT ;
		BufCnt_t_Jolt	: DINT ;
		Cnt_t_Jolt	: DINT ;
		s_set_Sum_dbl_1	: UDINT ;
		s_set_Sum_dbl_2	: UDINT ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		Ts_2_dbl_1	: UDINT ;
		Ts_2_dbl_2	: UDINT ;
		Ts_4_dbl_1	: UDINT ;
		Ts_4_dbl_2	: UDINT ;
		Timer	: REAL ;
		Timer_dbl_1	: UDINT ;
		Timer_dbl_2	: UDINT ;
		Timer_Ts	: REAL ;
		Timer_Ts_dbl_1	: UDINT ;
		Timer_Ts_dbl_2	: UDINT ;
		T_cycle	: REAL ;
		T_cycle_dbl_1	: UDINT ;
		T_cycle_dbl_2	: UDINT ;
		Const_T_cycle	: BOOL ;
		CounterState	: UINT ;
		musecs_old	: UDINT ;
		SysTicks_old	: UDINT ;
		enable_old	: BOOL ;
		Bootkey_T	: UDINT ;
		Bootkey	: UDINT ;
		s_period_ext	: REAL ;
		PeriodicMode_ext	: INT ;
		NewPeriod_ext_out	: SINT ;
	END_STRUCT;
	HCRs_Cont_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		vCyl_sup_pos	: REAL ;
		vCyl_sup_neg	: REAL ;
		vCyl_sup	: REAL ;
		s_set_old	: REAL ;
		s_act_old	: REAL ;
		v_act	: REAL ;
		PID_s_bias	: USINT ;
		Qy_s_bias	: REAL ;
		yValve_bias	: REAL ;
		K_yValve	: REAL ;
		vCyl_bias	: REAL ;
		vCyl_sum	: REAL ;
		vCyl_abs	: REAL ;
		vCyl_rel	: REAL ;
		Y_bias	: REAL ;
		Oset_Mode	: USINT ;
		Qy_Oset_s_pos	: REAL ;
		Qy_Oset_s_neg	: REAL ;
		Qy_I_Switch	: USINT ;
		Qy_I	: REAL ;
		MoveDir_I	: SINT ;
		vMode	: USINT ;
		v_lim	: REAL ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRSystemPara_typ : STRUCT
		pSupply_max	: REAL ;
		pSupply	: REAL ;
		pTank	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
	END_STRUCT;
	HCRValvePara_typ : STRUCT
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		pDelta_nom	: REAL ;
		yValve_0	: REAL ;
		Neg_y	: BOOL ;
	END_STRUCT;
	HCRPIDPara_typ : STRUCT
		WX_min	: REAL ;
		WX_max	: REAL ;
		Kw	: REAL ;
		Kp	: REAL ;
		Tn_pos	: REAL ;
		Tn_neg	: REAL ;
		K_Fbk	: REAL ;
		d_mode	: USINT ;
		Tv	: REAL ;
		Tf	: REAL ;
		dY_max	: REAL ;
		Y_min	: REAL ;
		Y_max	: REAL ;
	END_STRUCT;
	HCRPID_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		WX_min	: REAL ;
		WX_max	: REAL ;
		_100_WX_delta_dbl_1	: UDINT ;
		_100_WX_delta_dbl_2	: UDINT ;
		W_norm_dbl_1	: UDINT ;
		W_norm_dbl_2	: UDINT ;
		X_norm_dbl_1	: UDINT ;
		X_norm_dbl_2	: UDINT ;
		Invert	: BOOL ;
		e_norm_dbl_1	: UDINT ;
		e_norm_dbl_2	: UDINT ;
		e_DTi	: REAL ;
		Kp	: REAL ;
		Kw	: REAL ;
		Kp_Kw_1_dbl_1	: UDINT ;
		Kp_Kw_1_dbl_2	: UDINT ;
		Kp_intrude	: REAL ;
		P_intrude	: BOOL ;
		Yp_dbl_1	: UDINT ;
		Yp_dbl_2	: UDINT ;
		Ts	: DINT ;
		Ts_dbl_1	: UDINT ;
		Ts_dbl_2	: UDINT ;
		Tn_pos_dbl_1	: UDINT ;
		Tn_pos_dbl_2	: UDINT ;
		Tn_neg_dbl_1	: UDINT ;
		Tn_neg_dbl_2	: UDINT ;
		Kp_Ts_Tn_pos_dbl_1	: UDINT ;
		Kp_Ts_Tn_pos_dbl_2	: UDINT ;
		Kp_Ts_Tn_neg_dbl_1	: UDINT ;
		Kp_Ts_Tn_neg_dbl_2	: UDINT ;
		Yi_load	: REAL ;
		I_load	: SINT ;
		Y_bias	: REAL ;
		PID_bias	: USINT ;
		Ypi_limit	: REAL ;
		PI_limit	: USINT ;
		Yi_dbl_1	: UDINT ;
		Yi_dbl_2	: UDINT ;
		Yi_pos_last_dbl_1	: UDINT ;
		Yi_pos_last_dbl_2	: UDINT ;
		Yi_neg_last_dbl_1	: UDINT ;
		Yi_neg_last_dbl_2	: UDINT ;
		I_Switch_Sign	: SINT ;
		I_Switch	: SINT ;
		I_Switch_old	: SINT ;
		I_Switch_off	: SINT ;
		d_mode	: USINT ;
		D	: REAL ;
		Tv_dbl_1	: UDINT ;
		Tv_dbl_2	: UDINT ;
		Tf_dbl_1	: UDINT ;
		Tf_dbl_2	: UDINT ;
		x1d_dbl_1	: UDINT ;
		x1d_dbl_2	: UDINT ;
		x2d_dbl_1	: UDINT ;
		x2d_dbl_2	: UDINT ;
		a0d_dbl_1	: UDINT ;
		a0d_dbl_2	: UDINT ;
		a1d_dbl_1	: UDINT ;
		a1d_dbl_2	: UDINT ;
		b0d_dbl_1	: UDINT ;
		b0d_dbl_2	: UDINT ;
		b1d_dbl_1	: UDINT ;
		b1d_dbl_2	: UDINT ;
		Yd_dbl_1	: UDINT ;
		Yd_dbl_2	: UDINT ;
		Reset_Yd	: USINT ;
		Yadd	: REAL ;
		Y_min	: REAL ;
		Y_max	: REAL ;
		Ytotal_dbl_1	: UDINT ;
		Ytotal_dbl_2	: UDINT ;
		Ylim_dbl_1	: UDINT ;
		Ylim_dbl_2	: UDINT ;
		Y_Fbk_ext	: REAL ;
		Fbk_mode	: USINT ;
		K_Fbk_Kp_dbl_1	: UDINT ;
		K_Fbk_Kp_dbl_2	: UDINT ;
		e_Fbk_dbl_1	: UDINT ;
		e_Fbk_dbl_2	: UDINT ;
		dY_max	: REAL ;
		dY_max_Ts_dbl_1	: UDINT ;
		dY_max_Ts_dbl_2	: UDINT ;
		Y_dbl_1	: UDINT ;
		Y_dbl_2	: UDINT ;
		UpdateValid	: BOOL ;
		enable_old	: BOOL ;
		Bootkey_CM	: UDINT ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRvyServo_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Alpha	: REAL ;
		vCyl_sup_pos	: REAL ;
		vCyl_sup_neg	: REAL ;
		vCyl_sup	: REAL ;
		vCyl_max_pos	: REAL ;
		vCyl_max_neg	: REAL ;
		vCyl_max	: REAL ;
		vCyl_sum	: REAL ;
		vCyl_thresh	: REAL ;
		vCyl_nOverlap_pos	: REAL ;
		vCyl_nOverlap_neg	: REAL ;
		vCyl_old	: REAL ;
		pSupply_max	: REAL ;
		pSupply	: REAL ;
		pTank	: REAL ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
		yValve_zero	: REAL ;
		yValve_min	: REAL ;
		Neg_y	: BOOL ;
		yValve_0	: REAL ;
		yValve_0_dbl_1	: UDINT ;
		yValve_0_dbl_2	: UDINT ;
		K_pos_dbl_1	: UDINT ;
		K_pos_dbl_2	: UDINT ;
		K_neg_dbl_1	: UDINT ;
		K_neg_dbl_2	: UDINT ;
		K_yValve	: REAL ;
		K_yValve_dbl_1	: UDINT ;
		K_yValve_dbl_2	: UDINT ;
		K_nOverlap_pos_dbl_1	: UDINT ;
		K_nOverlap_pos_dbl_2	: UDINT ;
		K_nOverlap_neg_dbl_1	: UDINT ;
		K_nOverlap_neg_dbl_2	: UDINT ;
		S_nOverlap_dbl_1	: UDINT ;
		S_nOverlap_dbl_2	: UDINT ;
		pDelta_min_dbl_1	: UDINT ;
		pDelta_min_dbl_2	: UDINT ;
		vCyl_sup_100_dbl_1	: UDINT ;
		vCyl_sup_100_dbl_2	: UDINT ;
		vMode	: USINT ;
		vCyl_lim	: REAL ;
		Update_old	: USINT ;
		Update_2	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRvyServo_Fp_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		Alpha	: REAL ;
		vCyl_sup_pos	: REAL ;
		vCyl_sup_neg	: REAL ;
		vCyl_sup	: REAL ;
		vCyl_max_pos	: REAL ;
		vCyl_max_neg	: REAL ;
		vCyl_max	: REAL ;
		vCyl_sum	: REAL ;
		vCyl_thresh	: REAL ;
		vCyl_nOverlap_pos	: REAL ;
		vCyl_nOverlap_neg	: REAL ;
		vCyl_old	: REAL ;
		pSupply_max	: REAL ;
		pSupply	: REAL ;
		pTank	: REAL ;
		Qnom_pos	: REAL ;
		Qnom_neg	: REAL ;
		A_Cyl_pos	: REAL ;
		A_Cyl_neg	: REAL ;
		pCyl_pos	: REAL ;
		pCyl_neg	: REAL ;
		yValve_zero	: REAL ;
		yValve_min	: REAL ;
		Neg_y	: BOOL ;
		yValve_0	: REAL ;
		yValve_0_dbl_1	: UDINT ;
		yValve_0_dbl_2	: UDINT ;
		Supply_Tank_pos_dbl_1	: UDINT ;
		Supply_Tank_pos_dbl_2	: UDINT ;
		Supply_Tank_neg_dbl_1	: UDINT ;
		Supply_Tank_neg_dbl_2	: UDINT ;
		Qn_pos_2_A_neg_2_dbl_1	: UDINT ;
		Qn_pos_2_A_neg_2_dbl_2	: UDINT ;
		Qn_neg_2_A_pos_2_dbl_1	: UDINT ;
		Qn_neg_2_A_pos_2_dbl_2	: UDINT ;
		Qn_pos_2_A_neg_3_dbl_1	: UDINT ;
		Qn_pos_2_A_neg_3_dbl_2	: UDINT ;
		Qn_neg_2_A_pos_3_dbl_1	: UDINT ;
		Qn_neg_2_A_pos_3_dbl_2	: UDINT ;
		K_dbl_1	: UDINT ;
		K_dbl_2	: UDINT ;
		K_pos_dbl_1	: UDINT ;
		K_pos_dbl_2	: UDINT ;
		K_neg_dbl_1	: UDINT ;
		K_neg_dbl_2	: UDINT ;
		K_yValve	: REAL ;
		K_yValve_dbl_1	: UDINT ;
		K_yValve_dbl_2	: UDINT ;
		K_nOverlap_dbl_1	: UDINT ;
		K_nOverlap_dbl_2	: UDINT ;
		K_nOverlap_pos_dbl_1	: UDINT ;
		K_nOverlap_pos_dbl_2	: UDINT ;
		K_nOverlap_neg_dbl_1	: UDINT ;
		K_nOverlap_neg_dbl_2	: UDINT ;
		S_nOverlap_dbl_1	: UDINT ;
		S_nOverlap_dbl_2	: UDINT ;
		pCyl_pos_Numer_pos_dbl_1	: UDINT ;
		pCyl_pos_Numer_pos_dbl_2	: UDINT ;
		pCyl_neg_Numer_pos_dbl_1	: UDINT ;
		pCyl_neg_Numer_pos_dbl_2	: UDINT ;
		pCyl_pos_Numer_neg_dbl_1	: UDINT ;
		pCyl_pos_Numer_neg_dbl_2	: UDINT ;
		pCyl_neg_Numer_neg_dbl_1	: UDINT ;
		pCyl_neg_Numer_neg_dbl_2	: UDINT ;
		pCyl_Denom_dbl_1	: UDINT ;
		pCyl_Denom_dbl_2	: UDINT ;
		F_Delta_min_dbl_1	: UDINT ;
		F_Delta_min_dbl_2	: UDINT ;
		vCyl_sup_100_dbl_1	: UDINT ;
		vCyl_sup_100_dbl_2	: UDINT ;
		vMode	: USINT ;
		vCyl_lim	: REAL ;
		Update_old	: USINT ;
		Update_2	: USINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRLinSpline_Internal_typ : STRUCT
		statusTxt	: STRING[80] ;
		AscendingX_o	: SINT ;
		AscendingXs_o	: SINT ;
		AscendingY_o	: SINT ;
		AscendingYs_o	: SINT ;
		AscendingX_d	: SINT ;
		AscendingXs_d	: SINT ;
		AscendingY_d	: SINT ;
		AscendingYs_d	: SINT ;
		Rness_crit	: REAL ;
		RnessP	: REAL ;
		RnessR	: REAL ;
		_Smooth1_dbl_1	: UDINT ;
		_Smooth1_dbl_2	: UDINT ;
		_1Smooth_dbl_1	: UDINT ;
		_1Smooth_dbl_2	: UDINT ;
		_1Smooth1_dbl_1	: UDINT ;
		_1Smooth1_dbl_2	: UDINT ;
		enable_old	: BOOL ;
		Bootkey	: UDINT ;
	END_STRUCT;
	HCRNode_Internal_typ : STRUCT
		X	: REAL ;
		Y	: REAL ;
		X1	: REAL ;
		X2	: REAL ;
		Rness_crit	: REAL ;
		X1_dbl_1	: UDINT ;
		X1_dbl_2	: UDINT ;
		X2_dbl_1	: UDINT ;
		X2_dbl_2	: UDINT ;
		deltaX_dbl_1	: UDINT ;
		deltaX_dbl_2	: UDINT ;
		deltaY_dbl_1	: UDINT ;
		deltaY_dbl_2	: UDINT ;
		k_dbl_1	: UDINT ;
		k_dbl_2	: UDINT ;
		d_dbl_1	: UDINT ;
		d_dbl_2	: UDINT ;
	END_STRUCT;
	HCRNode_typ : STRUCT
		X	: REAL ;
		Y	: REAL ;
		Internal	: HCRNode_Internal_typ ;
	END_STRUCT;
END_TYPE