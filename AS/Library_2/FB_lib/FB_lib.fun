...
{REDUND_ERROR} FUNCTION_BLOCK FBio					(*copies the input and output data areas between the B&R card and the specified buffers; asynchronous execution*)
	VAR_INPUT
...
{REDUND_ERROR} FUNCTION_BLOCK FBStart				(*starts the I/O exchange process*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
	END_VAR
...
{REDUND_ERROR} FUNCTION_BLOCK FBStop				(*stops the I/O exchange process*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
	END_VAR
...
{REDUND_ERROR} FUNCTION_BLOCK FBSlaveStat			(*returns the current slave status*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBInit				(*searches and initializes the specified device*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBSlaveExtStat		(*returns the exact status of a slave; asynchronous execution*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBConfig				(*downloads the configuration; asynchronous execution*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBDialog				(*send messages to the B&R card; asynchronous execution*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		DevIdent	:UDINT;			(*device identifier returned from FBInit*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBGetMessage           (*sends a message to the B&R card and does not wait for an answer; asynchronous execution*)
	VAR_INPUT
		enable	:BOOL;                 (*enables execution*)
...
{REDUND_ERROR} FUNCTION_BLOCK FBPutMessage            (*reads out a message from the B&R card; asynchronous execution*)
	VAR_INPUT
		enable	:BOOL;                 (*enables execution*)
		DevIdent	:UDINT;            (*device identifier returned from DPMInit*)
		Message	:UDINT;                (*pointer to the message structure*)
