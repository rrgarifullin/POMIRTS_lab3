...
FUNCTION wcsconv : UDINT						(*converts one string to a different format*)
	VAR_INPUT	
	...
	END_VAR
END_FUNCTION
FUNCTION wcsncpy : UDINT						(*copies a maximum of n characters from the pSource WC string to the pDestination address*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION wcsncat : UDINT						(*appends a maximum of n characters from the pSource string to the pDestination string*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION wcscat : UDINT							(*connects two WC strings to each other (concatenation)*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*string to be appended to, given as a pointer*)
		pSource	:REFERENCE TO UINT;				(*string to be appended, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION wcscmp : DINT							(*compares two 16-bit character strings*)
	VAR_INPUT
		pUcstr1	:REFERENCE TO UINT;				(*first string, given as a pointer*)
		pUcstr2	:REFERENCE TO UINT;				(*second string, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION wcslen : UDINT							(*determines the length of a WC string*)
	VAR_INPUT
		pwcString	:REFERENCE TO UINT;			(*string whose length should be determined, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION wcsset : UDINT							(*appends the pSource string to the pDestination string*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION wcsncmp : DINT							(*compares two 16-bit strings*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION wcscpy : UDINT							(*copies a WC string*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*pointer to the target memory for the string*)
		pSource	:REFERENCE TO UINT;				(*string to be copied, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION wcsrchr : UDINT						(*searches for the last occurrence of a certain character in a WC string*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*character string being searched, given as a pointer*)
		symbol	:UINT;							(*character being looked for*)
	END_VAR
END_FUNCTION
FUNCTION wcschr : UDINT							(*searches for the first occurrence of a certain character*)
	VAR_INPUT
