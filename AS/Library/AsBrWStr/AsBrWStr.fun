...
FUNCTION brwcsconv : UDINT						(*converts one string to a different format*)
	VAR_INPUT	
	...
	END_VAR
END_FUNCTION
FUNCTION brwcsncpy : UDINT						(*copies a maximum of n characters from the pSource WC string to the pDestination address*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION brwcsncat : UDINT						(*appends a maximum of n characters from the pSource string to the pDestination string*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION brwcscat : UDINT						(*connects two WC strings to each other (concatenation)*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*string to be appended to, given as a pointer*)
		pSource	:REFERENCE TO UINT;				(*string to be appended, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION brwcscmp : DINT						(*compares two 16-bit character strings*)
	VAR_INPUT
		pUcstr1	:REFERENCE TO UINT;				(*first string, given as a pointer*)
		pUcstr2	:REFERENCE TO UINT;				(*second string, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION brwcslen : UDINT						(*determines the length of a WC string*)
	VAR_INPUT
		pwcString	:REFERENCE TO UINT;			(*string whose length should be determined, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION brwcsset : UDINT						(*appends the pSource string to the pDestination string*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION brwcsncmp : DINT						(*compares two 16-bit strings*)
	VAR_INPUT
	...
	END_VAR
END_FUNCTION
FUNCTION brwcscpy : UDINT						(*copies a WC string*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*pointer to the target memory for the string*)
		pSource	:REFERENCE TO UINT;				(*string to be copied, given as a pointer*)
	END_VAR
END_FUNCTION
FUNCTION brwcsrchr : UDINT						(*searches for the last occurrence of a certain character in a WC string*)
	VAR_INPUT
		pDestination	:REFERENCE TO UINT;		(*character string being searched, given as a pointer*)
		symbol	:UINT;							(*character being looked for*)
	END_VAR
END_FUNCTION
FUNCTION brwcschr : UDINT						(*searches for the first occurrence of a certain character*)
	VAR_INPUT
