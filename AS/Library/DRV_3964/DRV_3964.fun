...
FUNCTION_BLOCK S39SRk512		(*receives commands from the master to read or write data blocks*)
	VAR_INPUT
...
FUNCTION_BLOCK S39SClose		(*enables the interface and restores it to its original configuration before S39SOpen() was called*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		ident	:UDINT;			(*identifier returned from S39SOpen()*)
	END_VAR	
...
FUNCTION_BLOCK S39Slave			(*receives commands from the master to read or write data blocks*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		ident	:UDINT;			(*identifier returned from S39SOpen()*)
		mode	:USINT;			(*mode: s39MODE_DISABLE, s39MODE_RECV_ONLY, s39MODE_SEND_ONLY, s39MODE_SEND_RECV*)
...
FUNCTION_BLOCK S39SOpen			(*initializes the interface defined with the pdevice and pmode parameters*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
