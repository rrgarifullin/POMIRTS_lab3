 ...
 * Author: B+R
 ********************************************************************
 * Functions and function blocks of library DRVABDF1
 ********************************************************************)
                                                                      
FUNCTION_BLOCK ABDF1Master			(*operates the DF1 protocol (full-duplex) at runtime*)
	VAR_INPUT
...
FUNCTION_BLOCK ABDF1SOpen			(*returns the ident for ABDF1Slave(), ABDF1SClose(), ABDF1Master() and ABDF1MClose()*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		pDevice	:UDINT;				(*device description string*)
		pMode	:UDINT;				(*mode description string*)
...
FUNCTION_BLOCK ABDF1Slave			(*operates the DF1 protocol (full-duplex) at runtime*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
...
FUNCTION_BLOCK ABDF1SClose			(*closes the driver environment, after which the ABDF1SOpen() FBK can be called again*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		ident	:UDINT;				(*ident from the ABDF1SOpen() FBK*)
	END_VAR
...
FUNCTION_BLOCK ABDF1MClose			(*closes the driver environment, after which the ABDF1MOpen() FBK can be called again*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		ident	:UDINT;				(*ident from the ABDF1MOpen() FBK*)
	END_VAR
