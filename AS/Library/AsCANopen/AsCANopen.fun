...
********************************************************************)
 ...
 FUNCTION_BLOCK CANopenEnableCyclicSync (*enables or disables cyclic SYNC messages*)
	VAR_INPUT
...
FUNCTION_BLOCK CANopenGetLocalNodeID 	(*queries the local node address*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenGetKernelState 	(*queries the status of the CANopen kernel on the local PLC*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenRegisterCOBID 	(*registers or deletes receive of PDOs and CAN layer 2 messages using network layer*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
...
FUNCTION_BLOCK CANopenPDORead8 			(*reads PDOs and CAN layer 2 messages from input buffer of network layer*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		cobid : UINT; 					(*COBID (CAN identifier)*)
...
FUNCTION_BLOCK CANopenPDOWrite8 		(*writes PDOs and CAN layer 2 messages to network layer*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		cobid : UINT; 					(*COBID (CAN identifier)*)
...
FUNCTION_BLOCK CANopenSDORead8 			(*reads object entries of a node using SDO transfer*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
...
FUNCTION_BLOCK CANopenSDOWrite8	 		(*writes object entries of a node using SDO transfer*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
...
FUNCTION_BLOCK CANopenGetState 			(*queries the status of a node*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		node : USINT; 					(*node (1-127 or 0 for local OD)*)
...
FUNCTION_BLOCK CANopenNMT 				(*sends NMT messages*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		node : USINT; 					(*node (1-127 or 0 for local OD)*)
...
FUNCTION_BLOCK CANopenRecvEmcyDev 		(*reads a received emergency message (specific node)*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		node : USINT; 					(*node (1-127 or 0 for local OD)*)
...
FUNCTION_BLOCK CANopenRecvEmcy 			(*reads a received emergency message (any node)*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenWriteEmcy 		(*sends an emergency message*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		emcy_err_code : UINT; 			(*emergency error information (CiA Draft Standard 301)*)
...
FUNCTION_BLOCK CANopenRecvBootupDev 	(*reads a received boot message (specific node)*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		node : USINT; 					(*node (1-127 or 0 for local OD)*)
...
FUNCTION_BLOCK CANopenRecvBootup 		(*reads a received boot message (any node)*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenSendSync 			(*sends a single SYNC message*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenSDOReadData (*reads object entries of a node using SDO Blocktransfer*)
	VAR_INPUT
		enable 			: BOOL;		(*enables execution*)
		pDevice 		: UDINT;	(*device name given as a pointer*)
		node 			: USINT;	(*node (1-127 or 0 for local node)*)
...
FUNCTION_BLOCK CANopenSDOWriteData (*writes  object entries of a node using SDO Blocktransfer*)
	VAR_INPUT
		enable 			: BOOL;		(*enables execution*)
		pDevice 		: UDINT;	(*device name given as a pointer*)
		node 			: USINT;	(*node (1-127 or 0 for local OD)*)
...
FUNCTION CANopenSetSDOTimeout : UINT (*sets the timeout for SDO up- and downloads*)
	VAR_INPUT
		pDevice 		: UDINT;	(*device name given as a pointer*)
...
FUNCTION_BLOCK CANopenLayer2Read8 (*reads CAN layer 2 messages from input buffer of network layer*)
	VAR_INPUT
		enable 			: BOOL;		(*enables execution*)
		pDevice 		: UDINT;	(*device name given as a pointer*)
	END_VAR
...
FUNCTION_BLOCK CANopenGetSlaveState		(*queries the status of a node*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
...
FUNCTION_BLOCK CANopenActivateSlaveMonitoring	(*activates monitoring for the given CANopen slave in the CANopen master*)
	VAR_INPUT
		enable : BOOL; 					(*enables execution*)
		pDevice : UDINT; 				(*device name given as a pointer*)
		node : USINT; 					(*node (1-127)*)
