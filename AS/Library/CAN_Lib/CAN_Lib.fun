 ...
 * Author: B+R
 ********************************************************************
 * Functions and function blocks of library CAN_Lib
 ********************************************************************)
                                                                      
FUNCTION_BLOCK CANinfo			(*reads the statistical data from a CAN device*)
	VAR_INPUT
...
FUNCTION_BLOCK CANMulQueue		(*installs a queue for multiple CAN identifiers in the read direction*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANxMulQueue		(*installs a queue for multiple CAN identifiers in the read direction*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*us_ident from CANopen / CANMulOpen*)
		pCAN_id	:UDINT;			(*pointer to an array of CAN identifiers*)
...
FUNCTION_BLOCK CANMulOpen		(*initializes numerous CAN controllers and allocates the appropriate resources*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANMulrd			(*copies data from the queue to the buffer or sends an error in the status*)
	VAR_INPUT	
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANTime			(*reads the time stamp for a CAN object*)
	VAR_INPUT	
		enable		:BOOL;		(*enables execution*)
...
FUNCTION_BLOCK CANquwr			(*sends a CAN message with the given CAN identifier to the CAN controller for transmission*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK SetNdNr			(*saves the transferred node number and device description in the CRM*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK GetNdNr			(*reads the stored node number and device description from the CRM*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CMSmain			(*edits the list of CMS objects defined with the CMSinit() function*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CMSdlreq			(*transfers a "multiplexed domain" object of any data size from the client to the server*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CMSdlcon			(*enables a "multiplexed domain" object of any data length to be received by the server*)
	VAR_INPUT	
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANxnode			(*determines the node number set with the x-th CAN node switch*)
	VAR_INPUT	
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user Ident from CANopen()*)
	END_VAR
...
FUNCTION_BLOCK CANwrite			(*sends a CAN message with the given CAN identifier to the CAN controller for transmission*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user ident number from the CANopen() function*)
		can_id	:UDINT;			(*CAN identifier*)
...
FUNCTION_BLOCK CANupd			(*gives the response to an RTR request telegram to the send node*)
	VAR_INPUT					
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user ident from CANopen()*)
		can_id	:UDINT;			(*CAN identifier*)
...
FUNCTION_BLOCK CANngp			(*handles the Node Guarding Protocol according to CAL on the NMT slave*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user ident from CANopen()*)
		can_id	:UDINT;			(*CAN identifier*)
...
FUNCTION_BLOCK CANrwtab			(*edits the list of variables (variables table) defined with the CANdftab()*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANrtr			(*sends an RTR request frame on the CAN bus*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANread			(*reads CAN messages from the CAN controller*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user ident number from the CANopen() function*)
		can_id	:UDINT;			(*CAN identifier*)
...
FUNCTION_BLOCK CANrd			(*copies data from the queue to the buffer or sends an error in the status*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANxrd			(*copies data from the queue to the buffer or sends an error in the status*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		q_ident	:UDINT;			(*queue identifier from CANqueue()*)
		data_adr	:UDINT;		(*address of the data area where CAN data from the receive queue is stored*)
...
FUNCTION_BLOCK CANqueue			(*installs a queue for a CAN identifier in the read direction*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user Ident from CANopen()*)
...
FUNCTION_BLOCK CANopen			(*initializes the CAN controller and requests the necessary resources (e.g. memory)*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
...
FUNCTION_BLOCK CANnode			(*determines the CAN node number*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
	END_VAR
	VAR_OUTPUT
...
FUNCTION_BLOCK CANgetid			(*confirms the CAN identifier which the exception trigged in the context of an exception object*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		data_adr	:UDINT;		(*address of the data area in which the data received is stored*)
	END_VAR
...
FUNCTION_BLOCK CANexcep			(*activates a pre-installed exception object with the exception number exc_no*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user Ident from CANopen()*)
		can_id	:UDINT;			(*CAN identifier*)
...
FUNCTION_BLOCK CANdftab			(*supplies a variables list (variables table) defined by the user with data about the CAN bus*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user ident number from the CANopen() function*)
...
FUNCTION_BLOCK CANbtreg			(*configures a special bit timing register values for the CAN controller (Intel 82527)*)
	VAR_INPUT
		enable	:BOOL;			(*enables execution*)
		us_ident	:UDINT;		(*user Ident from CANopen()*)
