 ...
 * Author: B+R
 ********************************************************************
 * Functions and function blocks of library DRV_mbus
 ********************************************************************)
                                                                      
FUNCTION_BLOCK MBMCmd				(*operates the modbus protocol in runtime*)
	VAR_INPUT
...
FUNCTION_BLOCK MBMClose				(*closes the driver environment to enable the MBMOpen() FBK to be called again*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		ident	:UDINT;				(*identifier of the MBMOpen() FBK*)
	END_VAR
...
FUNCTION_BLOCK MBMaster				(*operates the modbus protocol in runtime*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		ident	:UDINT;				(*identifier of the MBMOpen() FBK*)
	END_VAR
...
FUNCTION_BLOCK MBMOpen				(*configures the interface and initializes the internal structures that are required*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
...
FUNCTION_BLOCK MBSClose				(*closes the driver environment to enable the MBSOpen() FBK to be called again*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
...
FUNCTION_BLOCK MBSlave				(*operates the modbus protocol in runtime*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		ident	:UDINT;				(*identifier of the MBSOpen() FBK*)
	END_VAR
...
FUNCTION_BLOCK MBSOpen				(*configures the interface and initializes the internal structures that are required*)
	VAR_INPUT
		enable	:BOOL;				(*enables execution*)
		pDevice	:UDINT;				(*device name given as a pointer*)
		pMode	:UDINT;				(*pointer to the mode description string*)
